...
{
	regex reg1("([+])([0-9]{3})([(])([0-9]{2})([)])([0-9]{3})([-])([0-9]{3})"),
		reg2("([+])([0-9]{3})([0-9]{2})([0-9]{3})([-])([0-9]{3})");
	return regex_match(brojTelefona, reg1) || regex_match(brojTelefona, reg2);
}
...
public:
	...
	void Sortiraj()
	{
		...
		while (promjena)
		{
			...
			{
				if ((_sortiranje == ASC && *_elementi1[i] > *_elementi1[i + 1]) || (_sortiranje == DESC && *_elementi1[i] < *_elementi1[i + 1]))
				{
...
public:
	...
	{
		if (_opis == obj._opis || _razred == obj._razred || _ocjena == obj._ocjena)
			return true;
		return false;
	}
...
public:
	...
	float Prosjek()
	{
		...
		{
			suma += _aktivnosti[i].GetElement2(i).GetOcjenu();
		}
	...
	void operator() (const char* imePrezime, string brojTelefona)
	{
		for (size_t i = 0; i < _ucenici.size(); i++)
		{
			if (strcmp(_ucenici[i].GetImePrezime(), imePrezime) == 0 || _ucenici[i].GetTelefon() == brojTelefona)
				throw exception("Nije dozvoljeno dodavanje istih ucenika!");
		}
		...
		for (size_t i = 0; i < _ucenici.size(); i++)
		{
			if (_ucenici[i].GetImePrezime() == imePrezime)
			{
				for (size_t j = 0; j < _ucenici[i].GetAktivnosti().GetTrenutno(); j++)
				{
					...
					if (_ucenici[i].GetAktivnosti().GetElement1(j) == predmet && _ucenici[i].GetAktivnosti().GetElement2(j).GetRazred() == aktivnost.GetRazred())
						return false;
...
int main() {
	...
	*/
	...
	if (gimnazijaMostar.SpasiUFajl("Gimnazija.txt"))								// moramo ukljuciti fstream biblioteku
